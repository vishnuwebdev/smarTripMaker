...
class VisaModel extends MY_Model {
    ...
    public function add(){
		...
		$visa_destination = $this->session->userdata['visa_destination'];
        $formData = array (
            "userId" 				=> $userid,
            ...
            "visaType" 				=> $visa_destination,
            "request_visa_id" 		=> bp_hash( $this->input->post("visa_type") ), 
			"request_location_id" 	=> bp_hash( $this->input->post("visa_country") ), 
            "createdOn" 			=> date('Y-m-d h:i:s')
        );
        $this->db->insert('visa', $formData);
        $orderId  = $this->db->insert_id();
        $unique_order = "STM-VISA";
        if($orderId < 10){
            $unique_order .= "00".$orderId;
        }else if($orderId > 9 && $orderId < 100){
            $unique_order .= "0".$orderId;
        }else{
            $unique_order .= $orderId;
        }
        $this->db->update("visa", ['unique_order'=>$unique_order], array('id'=>$orderId));
        return $orderId;
    }
...
function get_blog_list($table){
	...
	function location_list() {
		$this->db->select('location_id,location_location');
		$this->db->where('location_status', 'active');
		$this->db->from('location');
        $this->db->order_by("location_location", "asc");
		$query = $this->db->get();
		if($query->num_rows()>0){
			return $query->result();
		}else{
	...
	function visa_location_list() {
		$this->db->select('location_id,location_location');
		$this->db->where('visa_status', '1');
		$this->db->from('location');
        $this->db->order_by("location_location", "asc");
		$query = $this->db->get();
		if($query->num_rows()>0){
	...
	function visa_list_by_location_id( $location_id ){
		$this->db->select('id, visa_title, visa_amount');
		$this->db->where( array('visa_status' => 'active', 'country_id' => $location_id) );
    ...
    public function addVisaPayment($data){
        $transactionData = $this->getVisaTransaction($data['order_id']);
        $formData = array (
    ...
    public function getVisa($visaId){
        $this->db->select("*");
    ...
    public function getVisaTransaction($visaId){
        $this->db->select("*");
        $this->db->where('visa_id', $visaId);
        $query = $this->db->get('visa_payment_history');
        if($query->num_rows() > 0){
            return $query->row();
        }else{
            return false;
    ...
    public function addInitialTransaction($data){
        $this->db->insert('visa_payment_history', $data);
        return $this->db->insert_id();
    }
    
    public function updatePaymentTransaction($data){
        $transactionData = $this->getVisaTransaction($data['visa_id']);
        $this->db->where('id', $transactionData->id);
        $this->db->update('visa_payment_history', $data);
