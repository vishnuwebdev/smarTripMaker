...
class Hotel extends MX_Controller {
	...
	public function index() {
		...
		$category1 = $this->Hotel_Model->get_category("3");
		$category2 = $this->Hotel_Model->get_category("2");
		$package1 = $this->Hotel_Model->get_packages_by_category_id("3");
				                  ...
				                  <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
				                    <div class="htl-listing-price text-center">
				                      <div class="htl-price-list  mb-10">
				                        <div class="per-nt-price"><i class="icofont-rupee"></i>  '.$customer_fare .'<small class="d-block">per night</small> </div>
										 <div class="tw-nt-price">(<span><i class="icofont-rupee"></i>'.$customer_fare*$nights .'</span> for '.$nights.' nights)</div>
				                      </div>
		...
		foreach ( $bp_room_data as $bp_room_datas ) {
			if (in_array ( $bp_room_datas->RoomIndex, $bp_selected_room_index )) {
				...
				foreach ( $bp_room_datas->DayRates as $date_key => $bp_dates_value ) {
					$bp_select_room_array ["DayRates"] [$date_key] ['Amount'] = $bp_dates_value->Amount;
	...
	public function payment_request() {
		...
		// print_r($bp_booking_id);die;
		
		$totalFare = $_SESSION ['hotel'] ['amount'] ['customer_fare'];
		if ($_SESSION['payment_method'] == "customerCash") {
...
if ($this->session->userdata('Userlogin') != NULL) {
$customer_id = $this->session->userdata('Userlogin')['userData']->cust_id; }else {
		$customer_id = 0;
} 
