...
class Flight extends MX_Controller {
	...
	public function set_meals_data(){
		...
		if($type == 'insert'){
			$formData = array (
	            "sessionId" => $this->input->post ( 'sessionId' ),
	            "meal" => $this->input->post ( 'mealsName' ),
	            "price" => $this->input->post ( 'mealsPrice' ),
	            "created" => date('y-m-d h:i:s')
		    );
	    ...
	    $response = $this->Common_Model->insert_table ( "temp_meals_data", $formData);
			...
			$this->db->where('sessionId', $this->input->post ( 'sessionId' ));
			$this->db->where('meal',$this->input->post ( 'mealsName' ));
			$this->db->delete('temp_meals_data');
		...
		$query = $this->db->select('*')->where('sessionId', $sessionId)->get('temp_meals_data')->result();
	...
	public function save_customer_data() {
		...
		$baggageData = $this->Common_Model->get_table_order("*", "sessionId", $sessionid, "temp_baggage_data");
		$mealsData = $this->db->select('*')->where('sessionId', $sessionid)->get('temp_meals_data')->result();
		$mealsamount = 0;
		foreach($mealsData as $meals){
		...
		// print_r($getwayList->dsapayg_convenience_fee);
		// die;
		...
		// Need to manage convenienceAmount data from temp table.
		if ($this->input->post ( 'payment_method' )== "cc_avenue") {
			$transaction_fee = $_SESSION['flight']['Conv_fee'];
			if(isset($baggageData)){
				$baggageFee = ($baggageData->baggagePrice * $getwayList->dsapayg_convenience_fee) / 100;
				$transaction_fee =  round($transaction_fee + $baggageFee);
				$flight_total_fare = $_SESSION ['flight'] [$sessionid] ['flight_total_fare'] + $baggageData->baggagePrice;
				$_SESSION ['flight'] [$sessionid] ['flight_total_fare'] = $flight_total_fare;
			}

			if(isset($mealsData)){
				$mealsFee = ($mealsamount * $getwayList->dsapayg_convenience_fee) / 100;
				$transaction_fee =  round($transaction_fee + $mealsFee);
				$flight_total_fare = $_SESSION ['flight'] [$sessionid] ['flight_total_fare'] + $mealsamount;
				$_SESSION ['flight'] [$sessionid] ['flight_total_fare'] = $flight_total_fare;
			}
		} else {
			$transaction_fee = 0;
		...
		if ($searchData ['type'] == "Return" && $searchData ['IsDomestic'] == "true") {
			...
			$bookingData = array (
				...
				'fbook_baggage_amount'=>$baggageData->baggagePrice,
				'fbook_baggage_weight' =>$baggageData->baggageWeight,
				'fbook_meals_amount' =>$mealsamount,
