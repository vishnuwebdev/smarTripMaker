...
<!--					
	   ...
	   <!--check customer wallet -->
		  ...
		  <?php if($this->session->userdata("Userlogin") != NULL){
                        ...
                        <div class="col-md-12">
						<div class="checkbox">
							<label><input name="wallet_amount" class="term_condition" type="radio" value="customerCash"> Use Customer Wallet Amount <b>Available Balance is: </b><i class="icofont-rupee"></i><?php echo $bp_customer_total_balance; ?> </label>
                        </div>
						</div>
...
$(':checkbox[class=mealcheckbox]').on('click', function() {
	...
	if (confirmBox == true) {
		...
		var mealSelect = atob($(this).val());
		var mealSelect = mealSelect.split("#");
		...
		if(checked){
			...
			$.ajax({ 
				type: "POST", 
         		...
         		data: {'mealsName':mealSelect[0],'mealsPrice':mealSelect[1],'type':'insert','sessionId':sessionID,'type':'insert'},
         		dataType: "text",
				cache:false,
				success:
                function(data){
			...
			$.ajax({ 
				type: "POST", 
         		url: "<?php echo site_url(); ?>flight/set_meals_data", 
         		data: {'mealsName':mealSelect[0],'mealsPrice':mealSelect[1],'type':'insert','sessionId':sessionID,'type':'remove'},
         	});
		...
		localStorage.setItem("mealDynamic"+sessionID,getmeal);
		...
		localStorage.setItem("totalMeal"+sessionID, sum);
		
		

		let totalAmount = convFee  + parseInt(localStorage.getItem("totalMeal"+sessionID)) + parseInt(localStorage.getItem("baggagePrice"+sessionID) ? localStorage.getItem("baggagePrice"+sessionID) : 0);
        ...
        $(".finalamount").html(Math.round(totalAmount+totalConvenienceFee));
	}else{
		 return false;
	}
	...
	$.each(baggageJson, function (key, data) {
    	if(data.Weight == baggage){
			...
			$.ajax({
				...
				data: { 'baggageWeight': data.Weight, 'baggagePrice': data.Price,'sessionId':sessionID },
				dataType: "text",
				cache:false,
                ...
                function(data){
                	if(data){
                        ...
                        console.log(data,data.baggageWeight);
                        $(".baggage-weight").html(data.baggageWeight+'KG');
                        $(".baggage-price").html(data.baggagePrice);
                        let totalItemAmount = convFee + parseInt(data.baggagePrice);
						localStorage.setItem("baggagePrice"+sessionID,data.baggagePrice);

						let totalAmount = convFee  + parseInt(localStorage.getItem("totalMeal"+sessionID) ? localStorage.getItem("totalMeal"+sessionID) : 0) + parseInt(localStorage.getItem("baggagePrice"+sessionID));
						$("#loader").modal('hide');
