		...
		<div class="pageheader">
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li><a></i> <?php echo $this->lang->line ( "hotel_amenity_list" );?></a></li>
				</ul>
			</div>
		</div>
		<!-- row -->
		<div class="row">
			...
			<div class="col-md-12">
				...
				<section class="tile bp_shadow">
					<!-- tile header -->
						<?php
						$bp_active_tab_data['bp_active_tab']="hotel_amenity";
			...
			<?php }?>
					...
					<!-- tile body -->
					<!-- tile body -->
					<div class="mr-20"><a href="<?php echo site_url().$this->uri->segment ( "1" );?>/add_hotel_amenity" class="btn btn-greensea btn-ef btn-ef-7 btn-ef-7h mb-10 pull-right"><i class="fa fa-plus"></i> <?php echo $this->lang->line ( "add_hotel_amenity" );?></a></div>
					<div class="tile-body">
						<table class="table table-bordered bp_table">
							...
							<tbody>
									...
									<td><?php echo $bp->hohex_name; ?></td>
									 <td><i class="fa <?php echo $bp->hohex_icon; ?>"></i> &nbsp;&nbsp;<?php echo $bp->hohex_icon; ?></td>
									<td><?php if($bp->hohex_status=="active"){ ?><a href="<?php echo site_url().$this->uri->segment ( "1" );?>/update_hotel_amenity_status/?ref_id=<?php echo url_encode($bp->hohex_id); ?>&status=inactive" class="label label-success"><?php echo $this->lang->line ( "active" );?></a><?php }else{ ?><a
										href="<?php echo site_url().$this->uri->segment ( "1" );?>/update_hotel_amenity_status/?ref_id=<?php echo url_encode($bp->hohex_id); ?>&status=active" class="label label-danger"><?php echo $this->lang->line ( "inactive" );?></a><?php }?></td>
									<td><?php echo date_format(date_create($bp->hohex_update_date),"h:i A , d M Y");?></td>
									<td><a href="<?php echo site_url().$this->uri->segment ( "1" );?>/edit_hotel_amenity_detail/?ref_id=<?php echo url_encode($bp->hohex_id); ?>" class="btn btn-primary btn-rounded btn-ef btn-xs btn-ef-5 btn-ef-5b"><i class="fa fa-pencil"></i> <span><?php echo $this->lang->line ( "edit" );?></span></a>
										<button class="btn btn-danger btn-rounded btn-ef btn-ef-5 btn-ef-5b btn-xs" onclick="confirm_pop_up('<?php echo site_url().$this->uri->segment ( "1" );?>/delete_hotel_amenity/?ref_id=<?php echo url_encode($bp->hohex_id); ?>')">
